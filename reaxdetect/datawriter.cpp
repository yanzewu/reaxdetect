...
int ReaxDataWriter::ReadData(const string & path, ReaxReader & reader, Simulation * simulation)
{
	...
	if (string((char*)buffer, (char*)buffer + 8) != RWDXHEADER)return 0;//check file head
	...
	infile.read((char*)buffer, sizeof(int)); if (buffer[0] != BlockMark)return 0;
	...
	for (auto& s : reader.molecules) {
		...
		s.readBin(buffer);
		infile.read((char*)buffer, sizeof(int)); if (buffer[0] != ElementMark)return 0;//endmark check
	}
	infile.read((char*)buffer, sizeof(int)); if (buffer[0] != BlockMark)return 0;//Blockmark check
	...
	for (auto& reaction : reader.reactions) {
		...
		infile.read((char*)buffer, sizeof(int)); if (buffer[0] != ElementMark)return 0;
	}
	infile.read((char*)buffer, sizeof(int)); if (buffer[0] != BlockMark)return 0;
	...
	for (auto& fs : reader.fss) {
		...
		infile.read((char*)buffer, sizeof(ElementMark)); if (buffer[0] != ElementMark)return 0;//endmark check
		...
		infile.read((char*)buffer, sizeof(ElementMark)); if (buffer[0] != ElementMark)return 0;
	}
	infile.close();
	return 0;
}
...
{
	string crtpath = path::get_path(crtpath);
...
int ReaxDataWriter::WriteReport(const string & path, const ReaxAnalyzer & analyzer)
{
	...
	outfile << "Reactions\n";
	outfile << "index,\treaction,\tfreqplus,\tfreqminus,\tkp_real,km_real,\tkp_error-,kp_error+,\tkm_error-,km_error+\n";
	for (size_t i = 0; i < analyzer.reactions.size(); i++) {
		...
		outfile << analyzer.kp_err[0][i] << "," << analyzer.kp_err[1][i] << ",";
		outfile << analyzer.km_err[0][i] << "," << analyzer.km_err[1][i] << endl;
	}
...
int ReaxDataWriter::WriteKineticFile(const string & path, const ReaxAnalyzer & analyzer)
{
	...
	outfile << "ELEMENTS" << join(analyzer.species, "\n") << "END\n";

	//species
	outfile << "SPECIES" << join(analyzer.species, "\n") << "END\n";
